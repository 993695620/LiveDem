...
class VideoH264Decoder {
    ...
    func initDecoder() -> Bool {
        ...
        * kCVPixelBufferWidthKey/kCVPixelBufferHeightKey: 视频源的分辨率 width*height
        ...
        let imageBufferAttributes = [kCVPixelBufferPixelFormatTypeKey : kCVPixelFormatType_420YpCbCr8BiPlanarFullRange,
                                     kCVPixelBufferWidthKey: width,
                                     kCVPixelBufferHeightKey: height] as [CFString : Any]
    ...
    private func decodeByte(data: Data, size: UInt32) {
        ...
        let naluSize = size - 4
        let length: [UInt8] = [UInt8(truncatingIfNeeded: naluSize >> 24),
        ...
        data.suffix(from: 4).forEach { b in
            frameByte.append(b)
        }
        let bytes = frameByte
    ...
    private func decode(frame: [UInt8], size: UInt32) {
        ...
        let readyState = CMSampleBufferCreateReady(allocator: kCFAllocatorDefault, dataBuffer: blockBuffer, formatDescription: decodeDesc, sampleCount: CMItemCount(1), sampleTimingEntryCount: CMItemCount(), sampleTimingArray: nil, sampleSizeEntryCount: CMItemCount(1), sampleSizeArray: &sampleSizeArray, sampleBufferOut: &sampleBuffer)
        ...
        let decodeState = VTDecompressionSessionDecodeFrame(decompressionSession!, sampleBuffer: sampleBuffer!, flags: VTDecodeFrameFlags._EnableTemporalProcessing, frameRefcon: sourceFrame, infoFlagsOut: &infoFalg)
        if decodeState != 0 {
    ...
    private func setCallBack() {
        ...
        callback = { decompressionOutpuutRefCon, sourceFrameRrefCon, status, infoFlags, imageBuffer, pesentationTimeStamp, presentationDuration in
            
            let decoder: VideoH264Decoder = unsafeBitCast(decompressionOutpuutRefCon, to: VideoH264Decoder.self)
